...
echo "<div class='col-md-1'>";
    ...
    if ($num_product_image > 0) {
        // loop through all product images
            foreach ($stmt_product_image as $single_product_image) {
            // image name and source url
            $product_image_name = $single_product_image['name'];
            $source="../uploads/images/{$product_image_name}";
            echo "<img src='{$source}' class='product-img-thumb' data-img-id='{$single_product_image['id']}' />";
        }
    ...
    if ($num_product_image > 0) {
        ...
        // while ($row = $stmt_product_image->fetch(PDO::FETCH_ASSOC)){
            ...
            echo "<a href='{$source}' target='_blank' id='product-img-{$single_product_image['id']}' class='product-img {$show_product_img}'>";
